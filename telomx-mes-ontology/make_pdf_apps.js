// 앱별 온톨로지 활용 로드맵 — 아티팩트 HTML 을 A4 PDF 로 인쇄한다.
// 사용: node /root/telomx-mes-ontology/make_pdf_apps.js
const { chromium } = require('/root/caren/careand-www/.ds-sync/node_modules/playwright-core');
const fs = require('fs');
const os = require('os');
const path = require('path');

const SRC = '/root/telomx-mes-ontology/APPS-ROADMAP.html';
const OUT = '/root/telomx-mes-ontology/pdf/텔롬엑스-MES-앱별-온톨로지-활용로드맵-r1.0.pdf';
...
<style>body{margin:0;padding:0;font:14px -apple-system,BlinkMacSystemFont,sans-serif;background:#faf9f5;color:#141413}img{max-width:100%}[hidden]{display:none!important}</style>
  ...
  const tmp = path.join(os.tmpdir(), 'tx-apps.print.html');   // 원본 디렉터리를 더럽히지 않는다
  ...
  // 인쇄 전 화면 모드 확인용 스크린샷 1장(§1 막대그래프가 잘리지 않는지)
  await page.screenshot({ path: '/tmp/claude-0/-root/2d324ea2-5377-48fc-9911-6e5014341097/scratchpad/apps-roadmap.png', fullPage: false });
  ...
  await page.pdf({
    ...
    footerTemplate: `<div style="width:100%;font-size:7pt;color:#64706C;font-family:'IBM Plex Mono',monospace;padding:0 12mm;display:flex;justify-content:space-between">
      <span>앱별 온톨로지 활용 로드맵 · TX-ONT-APPS r1.0 · 2026-09-20</span><span><span class="pageNumber"></span> / <span class="totalPages"></span></span></div>`,
