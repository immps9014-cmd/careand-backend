...
class SignupRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // 가입 의도(보호자/가사요청자). guardian일 때만 의미. 미지정 시 care.
            'intent' => ['nullable', 'in:care,housekeeping'],
            // 어르신과의 관계는 '보호자(care)' 가입에만 필수. 가사요청자(housekeeping)는 불필요.
            'relation' => [
                Rule::requiredIf(fn () => $this->input('role') === 'guardian' && $this->input('intent') !== 'housekeeping'),
                'nullable',
                'string',
                'max:20',
            ],
